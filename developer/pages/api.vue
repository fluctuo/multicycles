<template>
  <div>
    <b-container>
      <b-row class="mt-3">
        <b-col cols="3">
          <nuxt-link to="/docs"><h2>Documentation</h2></nuxt-link>
          <h2>API Reference</h2>
          <ul class="list-unstyled">
            ...
            <li>
              <nuxt-link to="/api/limits">Limitations</nuxt-link>
            </li>
          </ul>
          ...
          <ul class="list-unstyled">
            <li v-for="query in queries(this.$store.state.introspection)" :key="query.name">
              <nuxt-link :to="{ name: 'api-type-value', params: { type: 'Query', value: query.name }}">{{ query.name }}</nuxt-link>
            </li>
          </ul>
          ...
          <ul class="list-unstyled">
            <li v-for="t in types(this.$store.state.introspection)" :key="t.name">
              <nuxt-link :to="{ name: 'api-type-value', params: { type: 'Type', value: t.name }}">{{ t.name }}</nuxt-link>
            </li>
          </ul>
          ...
          <ul class="list-unstyled">
            <li v-for="t in enums(this.$store.state.introspection)" :key="t.name">
              <nuxt-link :to="{ name: 'api-type-value', params: { type: 'Enum', value: t.name }}">{{ t.name }}</nuxt-link>
            </li>
          </ul>
        </b-col>
        <b-col cols="9">

...
;(async function() {
  ...
  // 176x196
  const masque = await Jimp.read('./calques/Masque-blanc-x2.png')

  // 90x44
  const shadow = await Jimp.read('./calques/Ombre@2x.png')
  const markerShadow = new Jimp(x, y, 0xffffffff)
  markerShadow.opacity(0)
  markerShadow.composite(shadow, (x - 90) / 2, y - shadow_pos)
